    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-16">
            ...
            <a href="{{ route('home') }}" class="flex items-center space-x-2">
                ...
                <span class="text-xl font-bold gradient-text hidden sm:inline">AIwithMir</span>
            </a>
            ...
            <div class="hidden md:flex items-center space-x-1">
                ...
                <a href="{{ route('projects') }}"
                    class="px-3 py-2 rounded-lg text-sm font-medium hover:bg-primary-50 hover:text-primary-600 transition {{ request()->routeIs('projects*') ? 'text-primary-600 bg-primary-50' : 'text-surface-600' }}">Projects</a>
                <a href="{{ route('blog') }}"
        ...
        <div id="mobileMenu" class="hidden md:hidden pb-4 space-y-1">
            <a href="{{ route('home') }}" class="block px-3 py-2 rounded-lg text-sm font-medium hover:bg-primary-50 text-surface-600">Home</a>
            <a href="{{ route('about') }}" class="block px-3 py-2 rounded-lg text-sm font-medium hover:bg-primary-50 text-surface-600">About</a>
            <a href="{{ route('services') }}" class="block px-3 py-2 rounded-lg text-sm font-medium hover:bg-primary-50 text-surface-600">Services</a>
            <a href="{{ route('projects') }}" class="block px-3 py-2 rounded-lg text-sm font-medium hover:bg-primary-50 text-surface-600">Projects</a>
            <a href="{{ route('blog') }}" class="block px-3 py-2 rounded-lg text-sm font-medium hover:bg-primary-50 text-surface-600">Blog</a>
            <a href="{{ route('contact') }}" class="block px-3 py-2 rounded-lg text-sm font-medium hover:bg-primary-50 text-surface-600">Contact</a>
            <hr class="border-surface-200 my-2">
            @guest
                <a href="{{ route('login') }}" class="block px-3 py-2 rounded-lg text-sm font-medium text-primary-600">Login</a>
            @else
                <a href="{{ route('admin.dashboard') }}" class="block px-3 py-2 rounded-lg text-sm font-medium text-primary-600">Admin</a>
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="block w-full text-left px-3 py-2 rounded-lg text-sm font-medium text-surface-600 hover:bg-surface-100">Logout</button>
                </form>
            @endguest
        </div>
    </div>
